<template>
  <div>
    <div v-if="set.set !== null">
      ...
      <div
        v-for="[catagoryName, catagory] in catagories"
        :key="catagoryName"
        class="no-print-indent"
      >
        <h2>{{ displayNames[catagoryName.replace(/-\(.*\)/, "")] }}</h2>
        <div
          v-for="(question, index) of catagory"
          :key="question.id"
          class="no-print-indent"
        >
      ...
      <ol class="indent">
        <li
          v-for="question in set.set.alphabetRound.questions"
          :key="question.id"
          class="alphabet-questions"
        >
          <p>{{ question.definition }}</p>
          <p><b>Answer:</b> {{ question.term }}</p>
        </li>
    ...
    <div class="set-generation no-print">
      <button @click="generateSet" class="generate-button">
        Regenerate Set
      </button>
      <div>
...
const catagories = computed(() => {
  ...
  return Object.entries(set.set.catagories).sort(([a], [b]) =>
    a.localeCompare(b)
  );
});

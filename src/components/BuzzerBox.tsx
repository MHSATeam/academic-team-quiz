...
export default function BuzzerBox() {
  ...
  useEffect(() => {
    const unsubscribe = RealtimeStatus.buzzerClick.subscribe(
      (buzzerMessage) => {
        if (
          buzzList.findIndex((buzz) => buzz.name === buzzerMessage.name) === -1
        ) {
    ...
    <div className="flex flex-col h-screen">
      <div className="flex flex-col justify-center gap-2 grow">
        ...
        <div className="flex flex-col text-center">
          {currentClick !== null &&
            buzzList
              .filter((buzz) => buzz.name !== currentClick.name)
              .sort((a, b) => a.timestamp - b.timestamp)
              .map((buzz) => {
                return (
                  <span key={buzz.name}>
                    {buzz.name}:{" "}
                    {(buzz.timestamp - currentClick.timestamp) / 1000} seconds
                    later
                  </span>
                );
              })}
